...
scan_cpd(x, window_sizes=None, alpha=0.05, n_boot=400, vote_threshold=0.5,
         min_window=15, max_window=None, block_length=None,
         taper="tukey", ipm="wasserstein", tolerance_distance=None, random_state=None,
         n_jobs=-1, return_all=True, *, change_type=None, batch_size=32)
...
| `block_length`      | Integer or `None`, default `None` | Block length used in the tapered block bootstrap. If `None`, it is selected automatically using the `n^(1/3)` rule.                                                                                                                                                  |
...
| `tolerance_distance` | Integer or `None`, default `None` | Distance used to merge nearby detections across different window sizes. If `None`, the smallest window size is used as the default merging distance.                                                                                                                |
...
| `n_jobs`            | Integer or `None`, default `-1`   | Number of Rust/Rayon worker threads used for parallel computation. The default `-1` uses all available CPU threads.                                                                                                                                                  |
...
The `scan_cpd` function returns a `ScanResult` object. This object collects the main outputs of the SCAN procedure in one place, including the final detected change points, detection scores, voting information, per-window diagnostics, threshold values, input parameters, and metadata.
...
scan_single_window(x, window_size, alpha=0.05, n_boot=400, block_length=None,
                   ...
                   change_type=None, batch_size=32)
```
