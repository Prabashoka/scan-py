"""Local discrepancy and localization statistics."""
...
def localize_cp(x: Iterable[float], change_type: str = "meanvar") -> int:
    """Localize a change point inside a flagged local region."""
    arr = _as_float_list(x)
    if len(arr) < 3:
        raise ValueError("x must contain at least 3 observations")
    return int(_scan_rust.localize_cp(arr, str(change_type)))
